...
In diesem projekt installiere ich einen Netzwerkweiten werbeblocker (Pi-hole) auf einem Raspberry Pi zero w mithilfe von Gemini Pro
...
![SSH Fehleranalyse und Login](versuch 1 (2).png)
...
Damit nicht jedes Endgerät manuell konfiguriert werden muss, wurde der DHCP-Server des Routers (Fritz!Box) umgestellt.

Navigation in die IPv4-Netzwerkeinstellungen der Fritz!Box.

Austausch des standardmäßigen lokalen DNS-Servers (IP des Routers) durch die statische IP des Raspberry Pi (192.168.178.108).

![FritzBox DNS Konfiguration](Screenshot 2026-07-19 205356.png)
(Hinweis für dich: Denk dran, das Bild hier noch durch eines mit der .108 am Ende auszutauschen!)
...
Neben der grundlegenden Erfahrung in der Linux-Kommandozeile (Bash) und der Fehlersuche bei SSH-Verbindungen, war die wichtigste
Erkenntnis die Architektur-Planung:
...
Pi unweigerlich zum Ausfall der Internetauflösung für alle Geräte im Netzwerk. In einem produktiven Firmennetzwerk würde ich zur Absicherung (Failover)
zwingend einen zweiten DNS-Server (z.B. einen weiteren Pi-hole) aufsetzen, um Hochverfügbarkeit (High Availability) zu garantieren.
